...
##
"""
Wrapper class for opertations on svn repositories.

Methods are provided to manage selective archiving of resource files.
...
class SvnAdmin(SvnWrapperBase):
    """Wrapper class for opertations on svn administrative operations on repositories.
    """
...
class SvnSandBoxAdmin(SvnWrapperBase):
    ...
    def __getUpdateCmd(self, projectDir, relProjectPath, prune=False, appendErrors=False):
        ...
        if self._setCvsRoot():
            ...
            targetPath = os.path.join(self.__sandBoxTopPath, projectDir)
            cmd = " cd " + targetPath + "; "
            cmd += "cvs -q -d " +  self._cvsRoot  + " update -C -d " + pF + relProjectPath + \
                self._getRedirect(fileNameOut=errPath, fileNameErr=errPath, append=appendErrors) + " ; "
        else:
            cmd = None
        return cmd
