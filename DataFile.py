...
import sys, os, os.path, time, datetime, filecmp, stat, shutil, smtplib, tempfile
...
#            p = [ '../' * len(l1) ]
    ...
    def __symLinkRelative(self):
        ...
        """
        if (not self.srcFileExists() or not os.path.isabs(self.srcPath)): return
        if (self.dstPath is  None): return
...
#            self.vout.write("++INFO - linking %s to %s\n" % (self.dstPath,rPath))
    ...
    def __copy(self,op="copy"):
        ...
        """
        if (not self.srcFileExists()): return
        if (not self.dstDirExists()):
            self.__mkdir(self.dstDirName)
    ...
    def __compare(self):
        ...
        """
        isSame=False
        if (not self.srcFileExists()): return isSame
        if (not self.dstFileExists()): return isSame        
        ...
        if (self.srcType  == self.dstType):
            ...
            if (os.access(f1, os.F_OK) and os.access(f2, os.F_OK) ):                
                ...
                if (f1 != self.srcPath): os.remove(f1)
                if (f2 != self.dstPath): os.remove(f2)                
            else:
    ...
    def __remove(self):
        ...
        """
        if (not self.srcFileExists()): return True
    ...
    def __timeStampCopy(self):
        """Reset the atime/mtimes of the destination using those of the
           source file.
        """
        if (self.srcStat is None): self.srcStat=self.__stat(self.srcPath)
    ...
    def __timeStampSet(self,fPath,yyyymmdd):
        ...
        """
        if (len(yyyymmdd) != 8 or not __exists(fPath)): return
#
    ...
    def __setTimeStamp(self,fPath,tObj):
        """Reset the atime/mtimes of the input file using the input timestamp.
        """
        if (tObj is None or not self.__exists(fPath)): return
    ...
    def setSrcFileMode(self,mode):
        if (not self.srcFileExists()): return False
        return self.__setFileMode(self.srcPath,mode)

    def setDstFileMode(self,mode):
        if (not self.dstFileExists()): return False
...
#
    ...
    def pr(self,fh=sys.stdout):
        ...
        if (self.dstPath is not None):
            ...
            if (self.dstStat is None): self.dstStat=self.__stat(self.dstPath)
